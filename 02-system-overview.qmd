...
Today, NAV Brasil has 1698 employees in 44 different units, providing aerodrome control services, non-radar approach, meteorology and aeronautical information for the respective locations.
...
These services are primarily organised along state boundaries and FIR (Flight Information Region) borders, with a number of limited cross-border agreements in place between adjacent airspaces and air traffic service units.    
...
# BRA ATCO increase
bra_atco_2019 <- 3126
bra_actos     <- c("2023" = 3677, "2024" = 3890)
...
The different behaviour suggests a difference in work force flexibility between the systems. Brazil reacted swiftly to the increase of air traffic ranging at `r ( ((1935139/1594442) - 1) * 100) |> round(1)`% higher than in 2019. 
...
At the end of 2023, the level reached about 90% of the pre-pandemc air traffic, and with 2024 the gap is closing to about 96%.
These recovery numbers are impacted by the geo-political developments. 
Due to the Russion invasion of Ukraine, a certain share of flights are currently banned to operate to/from Europe.

Both regions operate with similar operational concepts, procedures and supporting technology.
Considering the non-oceanic dimension of the airspace, Brazil services an area about 22% smaller than Europe. 
Brazil, with lower traffic density related to airspace, finds probably a more challenging cost-benefit ratio to maintain communication coverage and surveillance for regions with low-traffic.
The higher traffic density in Europe may cross all aspects of flight management.
In particular, the European region faces more considerable challenges in coordinating efforts to adress operational constraints and service the current demand.
...
tmp |> 
  ggplot() + 
  geom_step(aes(x = RANK, y = CUM_SHARE, group = REGION, color = REGION)) +
