...
The network analysis is work in progress.
Thus, the visualisation and section may change, and with a smaller data set.
:::
...
comb_deps_routes |> filter(RANK <= 150) |> 
  ggplot() + 
  geom_step(aes(x = RANK, y = CUM_SHARE, group = REGION))
