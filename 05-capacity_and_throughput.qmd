...
Only Curitiba and Porto Alegre maintained the same level as 2023. 
...
```{r}
#| label: prep-bli-pli-lppt-sbgr

#load data
sbgr_bli_pli <- arrow::read_parquet("./data/BRA-SBGR-THRU-BLI-PLI.parquet") |> 
  mutate(REG = "BRA")
lppt_bli_pli <- read_csv("./data/EUR-THRU-analytic-2019-2014-LPPT.csv", show_col_types = FALSE) |> 
  rename(BIN_HR = BIN, THRU_ARRS = ARRS, THRU_DEPS = DEPS, TOT_THRU = FLTS) |> 
  mutate( YEAR = lubridate::year(BIN_HR)
         ,MAX_CAP = 40, PLI = TOT_THRU >= 0.8 * MAX_CAP
         ,BLI = TOT_THRU >= 0.2 * MAX_CAP
         ,REG = "EUR")

comb_bli_pli <- bind_rows(sbgr_bli_pli, lppt_bli_pli)

years_to_show <- c(2019, 2023, 2024)

prep_ordered_thrus <- function(.thrus, .show_these_years = years_to_show){
  this_order <- .thrus |> 
    dplyr::filter(YEAR %in% .show_these_years) |> 
    dplyr::group_by(ICAO, YEAR)  |> 
    dplyr::arrange(desc(TOT_THRU), .by_group = TRUE) |> 
    dplyr::mutate(RANK = row_number()) |> ungroup() 
  this_order
  }

plot_ordered_thrus <- function(.ordered_thrus){
  this_plot <- .ordered_thrus |> 
    dplyr::mutate(YEAR = as.character(YEAR)) |> 
    
    ggplot2::ggplot(ggplot2::aes(x = RANK, y = TOT_THRU
...
cap_levels <-  tribble(~REG, ~ICAO, ~PLI_THR, ~BLI_THR, 
                     "BRA", "SBGR", 48 , 12, 
                     "EUR", "LPPT", 32, 8
                     )
...
comb_bli_pli |> 
  ...
  plot_ordered_thrus() + 
    facet_wrap(.~ ICAO) +
    labs(color = ggplot2::element_blank(), x = element_blank()
         ,y = "hourly throughput") +
    theme(legend.position = c(0.9,0.9)) +
    geom_hline(data = cap_levels, aes(yintercept = PLI_THR), linetype = "dashed") +
    geom_hline(data = cap_levels, aes(yintercept = BLI_THR), linetype = "dotted") +
    geom_text(
    data = cap_levels |> mutate(YEAR = "2024", LABEL = "peak (80% max cap)", X = 7600)
    ,aes(x = X, y = PLI_THR, label = LABEL),vjust = -1, show.legend = F, size = 8/.pt
    ) +
    geom_text(
    data = cap_levels |> mutate(YEAR = "2024", LABEL = "base (20% max cap)", X = 600)
    ,aes(x = X, y = BLI_THR, label = LABEL), vjust = -1, show.legend = F, size = 8/.pt
    , hjust = 0)
...
Comparing the achieved hourly throughput at Guarulhos (SBGR) and Lisbon (LPPT), @fig-bli-pli-lppt-sbgr depicts a higher level of peak throughput traffic numbers at SBGR.
The overall patterns are similar across the studied years.
Lisbon (LPPT) observed an increase in its overall traffic throughputs as the hourly throughputs for 2023 and 2024 exceed the pre-pandemic levels of 2019.
Guarulhos (SBGR) experienced a bounce-back with the continual increase in its hourly throughput between 2023 and 2024 catching up with the pre-COVID levels.

In terms of demand pressure, Lisbon observed for a higher share of hours with throughputs above 80% of its declared capacity than SBGR.
As the overall ordered throughput shows a gradual reduction gradient Guarulhos (SBGR) combined with a large spread of the peak and base level, there exists available capacity at the airport.
...
@fig-bli-pli-lppt-sbgr shows that multiple factors influence the interplay between the declared capacity and observed throughputs. 
...
eur_bli_pli <- list.files("data", pattern = "^EUR-BLI-PLI", full.names = T) |> 
  purrr::map(.f = ~ read_csv(.x, show_col_types = FALSE)) |> bind_rows() |> 
  mutate(REG = "EUR")

bli_pli     <- bind_rows(bli_pli_bra, eur_bli_pli)   # bra loaded with supp R scripts

bli_pli_plot(bli_pli, 2019:2024) +
  theme(legend.position = c(0.07, 0.8), legend.key.height = unit(4, "mm"))
```



@fig-bli-pli summarises the observed utilisation of the available (i.e., declared) capacity at the study airports in the different regions.
Based on this report, it appears that airports showing a BLI of higher than 0.75 observe a consistent level of demand across the operating hours, congestion due to peak operating conditions kick in with a PLI of 0.3. The lower range suggesting a strong banks, while higher levels of the PLI demonstrating a consistent use of the available capacity.
In Brazil, we observe a high utilisation of the capacity at Sao Paulo (SBSP) in 2023 and 2024 comparable to pre-COVID levels in 2019.
...
Rio de Janeiro observed moderate loads confirming the role of the airport within the Brazilian system. 
that both aerodromes are characterised by a relatively conservative and low capacity declaration.
The major hub in Brazil, SBGR shows a relatively high base-load-index (BLI), however rarely observed peak loads back in 2019.
Within the European context, a high utilisation of the available system capacity was observed for London Heathrow, Frankfurt, Gatwick, and Lisbon in 2019, 2023, and 2024 with a BLI above 0.8 and the associated PLI above 0.3 (top right quadrant).
This suggests that for many of these airports the daily traffic loads returned to similar levels of capacity utilisation than pre-COVID.
